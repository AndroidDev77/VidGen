...
export interface SceneAnalysisResult { schema_version:"1.0"; scene_id:UUID; sequence:number; source_start_ms:number; source_end_ms:number; summary:string; dramatic_purpose:string; observed_characters:CharacterCandidate[]; character_aliases:Record<string,string[]>; anonymous_speaker_references:string[]; location_candidates:LocationCandidate[]; state_changes:StateEvent[]; important_actions:AnalysisObservation[]; candidate_plot_beats:AnalysisPlotBeat[]; candidate_causal_links:BeatDependency[]; visual_motifs:string[]; direct_observations:AnalysisObservation[]; inferences:AnalysisInference[]; confidence:number; source_references:SourceReference[]; assumptions:StructuredNote[]; warnings:StructuredNote[]; unresolved_questions:string[] }
