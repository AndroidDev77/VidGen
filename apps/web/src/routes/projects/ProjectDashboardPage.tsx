...
export function ProjectDashboardPage(): JSX.Element {
  ...
  const status = useQuery({
    ...
    enabled: projectId !== "",
    // Stage progress lives only in this response, so the dashboard polls it
    // while a stage is in flight and stops once the pipeline settles. An
    // interval refetch keeps the last response in `status.data`, so the bar
    // updates in place rather than flickering through a loading state, and it
    // only fires while the tab is visible.
    refetchInterval: ({ state }) => stageProgressPollInterval(state.data, workflow.data),
  ...
  return (
    <div className={styles.stack}>
      ...
      <div className={styles.grid}>
        {status.data?.stage_progress && (
          <div className={styles.wide}>
            <StageProgressPanel progress={status.data.stage_progress} />
          </div>
        )}
