...
| T17b | Complete | T17 delivered the deterministic rendering library its roadmap row asked for, and that row stays Complete. What no row owned was *executing* a render: `RenderManifest` was constructed only by tests, T18 could queue a render-job row that nothing consumed, the parent workflow had no render stage, the documented `workers/render_job/main.py` did not exist, and the T24 Container Apps Job ran a command that inserted a queue row and exited. T17b is the corrective task that owns that boundary. `services/render_execution` is the single application service every entry point calls: it resolves and hashes the project's authoritative inputs, builds the canonical caption track and the immutable manifest, drives T17's `DeterministicRenderPipeline`, verifies the output, persists every canonical artifact through `AssetService`, and marks the render job complete only after all of that has committed. Execution is safe under concurrency: a conditional-update claim, a lease with heartbeats, stale-lease recovery, bounded attempts, durable checkpoints and idempotent completion. The parent workflow gains a render stage on the `render` task queue before T22; T18 queues an executable job, shows real progress and resolves the download to the persisted MP4; T22 evaluates exactly that render; and the Container Apps Job runs the real worker. |
...
| T24 | Implemented; staging validation pending | `infra/bicep` describes a reproducible, private-by-default Azure staging environment at resource-group scope: a VNet-integrated Container Apps environment with an internal load balancer; the web, API and Temporal worker as Container Apps; finite Container Apps Jobs for migration, out-of-band rendering, the private smoke test and administrative commands; PostgreSQL Flexible Server with VNet injection, enforced TLS and configurable SKU, storage, backups and redundancy; private Blob Storage with versioning, soft delete, lifecycle rules and no account key; Azure Managed Redis (`Microsoft.Cache/redisEnterprise`) behind a private endpoint; Key Vault with Azure RBAC authorisation and purge protection; six user-assigned managed identities with deterministic, least-privilege role assignments that never include Owner, Contributor, AcrPush or a Key Vault writer role; private endpoints and private DNS zones for every linked service; and Log Analytics plus workspace-based Application Insights carrying the existing T23 telemetry through a batching OTLP exporter. Public ingress is a single parameter, false in staging and in the production template, and the API is never external because production authentication does not exist yet. Images are pinned by base-image digest, run as non-root, carry commit-SHA OCI labels, contain FFmpeg and ffprobe, and are deployed by digest - never `latest`. Schema changes are applied exactly once by a dedicated manual job with parallelism one, no retry, a PostgreSQL advisory lock and a single-head assertion, before any new revision receives traffic; a failed migration blocks the traffic switch and rollback restores an application revision without ever downgrading the database. GitHub Actions authenticates through Azure OIDC federation with no client secret, and the private smoke test runs as a Container Apps Job inside the virtual network with every provider forced off. Implementation, automated review and CI are complete. T24 is **not** marked complete because the acceptance it claims - a private staging deploy, migrations run once, workers autoscaling - can only be demonstrated by a real deployment, and no Azure subscription or credential has been available. The blockers are listed in `infra/README.md`; the environment is deployed with the manually triggered `Deploy staging` workflow, and this row becomes Complete when that run succeeds. |
| T25 | Implemented; real-channel acceptance pending | `services/publisher` publishes an approved, current, T22-`PASS` render to the connected user's own YouTube channel through a restartable pipeline. Eligibility is proven from persisted rows before any OAuth refresh or YouTube request: a selected, complete, verified T17 render; a readable final MP4; an unrevoked T18 approval for that exact render job; a selected T22 run decided `PASS` with a matching `PASS` completion gate for that render identity; nothing invalidated after rendering; no T21 repair in `HUMAN_REVIEW_REQUIRED`; a caption track reached through the render job so it belongs to that lineage; a project-owned JPEG or PNG thumbnail; and a connection this owner controls with every required scope. Every URL, scope, limit, quota unit, retryable status and processing state lives once in a versioned capability registry with the date it was verified. OAuth is the web-server flow with PKCE S256, a one-time owner-bound hashed `state`, a sealed verifier, a redirect allowlist checked on both store and use, backend-only code exchange and server-resolved channel identity; refresh tokens are sealed with AES-256-GCM in a table separate from the connection, with a versioned, rotatable key. The resumable upload persists its sealed session before any media byte, advances only on YouTube's own confirmed offset, never revisits byte zero while a session can be resumed, and streams ranged reads rather than loading the MP4. An unprovable outcome stops at `HUMAN_REVIEW_REQUIRED` with the session evidence preserved rather than creating a second video. The video ID is persisted before the first processing poll; captions and the thumbnail run after the video exists so neither can cause a re-upload; every upload is private with subscriber notifications off and the synthetic-media disclosure set; and a visibility change is an explicit action that re-checks the render lineage and the T22 gate immediately before it runs and persists the privacy YouTube returns. Quota units are recorded on the existing T23 attempts as a typed usage quantity with zero monetary cost. The state machine is a database CHECK generated from the application's own transition table. T25 is **not** marked complete because the acceptance it claims - a real private test video, caption and thumbnail on a real channel - can only be demonstrated by the opt-in `tests/test_t25_real_youtube.py`, and no Google Cloud project, verified OAuth client or test channel has been available. Every fake-provider and mocked-adapter test passes. |
| T26 | Planned | Do not begin a later task until its dependencies and the current implementation are reconciled. |
...
- Utility and normal shots pass at 85 or higher. Hero shots pass at 90 or higher. These four scores - the three pass thresholds and the 75 targeted-repair floor - are deployment defaults a project may override through `visual_qa_thresholds` in its generation settings; an unset score keeps the deployment's. A floor above a pass score would describe a gate that repairs nothing - every shot that fails is already below the floor - so that combination is refused at the API boundary rather than stored; a project that lowers a pass score past the floor lowers the floor in the same write.
- A technical hard failure (black video, decode failure, duration error over 200 ms, or any deterministic hard-failure measurement) forces `FAIL` regardless of total score.
- A semantic hard-failure proposal from the evaluator (identity, count, location, props, action, anatomy, continuity) blocks only when the evaluator also scored that dimension below the `semantic_hard_failure_dimension_floor` (50). Otherwise the finding is kept as a warning and the recomputed score decides, so a shot scoring 95 with a minor anatomy flag passes.
- Codes listed in the thresholds' `warn_only_codes` are recorded as `warn_only:<CODE>` warnings and never block the shot or route a repair. The deployment sets the default with `VIDGEN_VISUAL_QA_WARN_ONLY_CODES`; a project overrides it outright through `visual_qa_warn_only_codes` in its generation settings, the same way the narration gate's warn-only set works. The resolved set is bound into the QA identity, so changing it starts a new evaluation rather than reinterpreting a stored one.
- A score from 75 to the pass threshold triggers targeted repair, unless every repair code the shot evidenced is tolerated - or the code its worst dimension would name is - in which case the shot passes on tolerance and records `warn_only:score_below_threshold`, rather than being handed a repair for the codes the project asked not to act on. A score below 75 triggers prompt simplification, a new seed, or composition split instead of a cosmetic repair. `PROMPT_TOO_COMPLEX` is added only when the recommended family is prompt simplification, and `TOO_MANY_CHARACTERS` only for composition split.
...
Unconditional hard failures are duration error over 200 ms, black video, decode failure, non-finite technical measurements, missing mandatory T19 identity evidence, or A/V material that cannot be normalized. The evaluator may also propose a hard failure for a wrong primary character, a missing required primary character, wrong character count, a missing mandatory action, a missing required prop, a contradicted approved location, a material screen-direction contradiction, severe broken face or anatomy, unintended readable text, or a contradictory persistent state; such a proposal blocks only when the evaluator's own score for that dimension is below the semantic floor, because a blocking flag on a dimension scored 95 contradicts the evidence the number summarizes. A hard failure that does block is structurally separate from a warning in the contracts, the recomputation, and the database, so it cannot be averaged away.
...
adding it leaves an existing render's identity unchanged rather than reshaping it in place. T17b's
production manifest builder (`services/render_execution/manifest_builder.py`) is that caller: every
render it assembles records the applicable QA result IDs, run IDs and policy version.
...
- A failed T20 video-QA result starts or resumes T21 for that shot through the `run_shot_repair`
  activity. The child adds `REPAIR_PLANNING -> REPAIRING -> ALTERNATE_PROVIDER ->
  FALLBACK_RENDERING -> REVALIDATING`, and terminates in `LOCKED`, `HUMAN_REVIEW_REQUIRED` or
  `REPAIR_FAILED`. A shot only reaches `LOCKED` when the selected output has passed its own T20
  evaluation. The repair is bounded, resumes existing provider operations rather than resubmitting
  them, honours cancellation between paid attempts, and never touches a sibling. Only compact IDs
  and codes enter Temporal history.
...
| T17b | Execute renders and integrate them | `services/render_execution/*`, `workers/render_job/main.py`, project workflow render stage, T18 download, T24 job command | T17, T20, T21 | A queued render job is executed end to end: manifest, captions, verified MP4, persisted assets, completed job, T22 run against that render, T18 download |
...
| T25 | Add YouTube publication | `services/publisher/*`, `src/vidgen/contracts/publication.py`, `src/vidgen/db/publication_models.py`, `src/vidgen/db/publication_repository.py`, `src/vidgen/security/envelope.py`, `packages/workflows/publication.py`, `workers/youtube_publisher/main.py`, `apps/api/routes/youtube_connections.py`, `apps/api/routes/publications.py`, `apps/web/src/components/PublicationPanel.tsx`, `scripts/connect_youtube.py`, `scripts/publish_youtube.py`, `scripts/inspect_publication.py` | T18, T22, T23, T24 | Private test video, thumbnail, and captions upload once; interrupted upload resumes from the server-confirmed offset |
...
| Separate Container Apps per queue family | One worker app polling `vidgen-projects`, plus a dedicated publisher worker on `vidgen-publisher` | Splitting by queue family would be speculative topology. The publisher is the one split the implementation actually needs: a multi-hour resumable upload must not hold a slot the rest of the pipeline is waiting on. |
| Container Apps Jobs for FFmpeg renders | The in-workflow T17b render stays a Temporal activity on the `render` task queue; a bounded manual render **job** runs `python -m workers.render_job.main --from-env` against an existing render-job ID for out-of-band re-renders | The workflow render is a durable step with retries, heartbeats and a completion gate. Forcing a continuously coordinated activity into a finite job would break that boundary. Both paths call the same executor, so the out-of-band job produces the same database state as the workflow. |
| Azure Service Bus | Not deployed | Nothing in the current implementation produces or consumes an external event. T25 publication runs on a dedicated Temporal task queue rather than an external bus, so it did not need one either. |
