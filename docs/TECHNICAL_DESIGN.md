...
- A score from 75 to the pass threshold triggers targeted repair. A score below 75 triggers prompt simplification, a new seed, or composition split instead of a cosmetic repair. `PROMPT_TOO_COMPLEX` is added only when the recommended family is prompt simplification, and `TOO_MANY_CHARACTERS` only for composition split.
