...
| Five `use_alter` foreign keys are never created on PostgreSQL | Every migration builds its tables with `Table.create()` one at a time; SQLAlchemy omits a `use_alter` constraint from `CREATE TABLE` on any backend that supports `ALTER TABLE`, and only `MetaData.create_all` emits the follow-up `ADD CONSTRAINT`. `fk_shots_selected_image_id_generated_images`, `fk_shots_selected_video_id_generated_videos`, `fk_animation_items_selected_generated_video`, `fk_visual_qa_runs_selected_result` and `fk_repair_runs_selected_attempt` therefore exist on SQLite and are missing on PostgreSQL, which is why the drift tests only pass on SQLite | A deployed database does not enforce that a selected image, video, QA result or repair attempt belongs to a real row | A follow-up migration that adds all five with `op.create_foreign_key(..., ondelete=...)` when the bind is not SQLite |
