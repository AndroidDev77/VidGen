...
| Two `use_alter` foreign keys are never created on PostgreSQL | `migrations/versions/0016_visual_qa.py` and `0017_repair_fallback.py` build their tables with `Table.create()` one at a time; SQLAlchemy omits a `use_alter` constraint from `CREATE TABLE` on any backend that supports `ALTER TABLE`, and only `MetaData.create_all` emits the follow-up `ADD CONSTRAINT`. `fk_visual_qa_runs_selected_result` and `fk_repair_runs_selected_attempt` therefore exist on SQLite and are missing on PostgreSQL | A deployed database does not enforce that a selected QA result or repair attempt belongs to a real row, and `alembic check` reports drift against a production database | A follow-up migration that adds both constraints with `op.create_foreign_key(..., ondelete="SET NULL")` when the bind is not SQLite |
