...
def test_project_api_and_resumable_upload(
    ...
    # browser can actually load. The path is root-relative so the browser
    # resolves it against its own origin rather than the API container's
    # hostname. The signature has to survive that rewrite.
    url = download.json()["url"]
    assert url.startswith("/api/v1/blobs/")
