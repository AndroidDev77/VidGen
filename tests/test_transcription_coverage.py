...
def test_tolerated_boundary_gap_counts_as_covered() -> None:
    coverage = calculate_coverage(
        [TimeInterval(start_seconds=0, end_seconds=1)],
        [TranscriptWord(text="hello", start_seconds=0.2, end_seconds=1)],
        gap_tolerance_seconds=0.25,
    )
    assert coverage.passed
    assert coverage.ratio == 1
...
def test_zero_voiced_duration_is_a_controlled_failure() -> None:
    with pytest.raises(ValueError, match="no voiced intervals"):
        calculate_coverage([], [])
